...
| `small-parts.stl` | — | 軸⌀7.85 ×4 / 抜け止めキャップ ×4 / 偏心ブッシュ ×4 / スペーサー ×10 | そのまま |

スペーサーは3種（4.5mm ×4、19mm ×2、15mm ×4）。ベアリングを軸方向に位置決めし、
軸上で滑るのを止めます。
...
- `ironDiameter` … こてグリップ（ゴム部）の最太部（928D-III実測22.0mm）。ボアは
  `clampClear`（片側0.3mm）を足した⌀22.6で、割りを締めて詰める
- `ironFrontDiameter` … グリップ先端側プラスチック部の直径（**要実測**）。ゴム部より
  細ければここに入れると下側`frontLen`（15mm）のボアが段付きになり、接触長が伸びる
...
- 向き: すべてSTLの向きのまま。柱は横倒しのまま印刷する（立てない）。
  ⌀7.85軸は垂直印刷なのでブリム推奨。強度が不安ならM8ボルトに置き換える
