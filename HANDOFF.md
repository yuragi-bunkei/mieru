...
1. **こて外径の反映（ブロッカー）**: ユーザーのこてはYIHUA製（型番未確定。候補: 926LED-IV / 928D-III / 908D。本人に確認中、返答待ち）。
   型番確定または実測後、`model.js` の `P.ironDiameter`（現在22.0mm）を差し替えて再生成する
2. **実印刷での嵌合検証**: 軸穴⌀8.3・ブッシュ穴⌀12.2・ベアリング転がり面は未印刷・未検証。
   きつい場合は `P.axleHole` / `P.bushHole` / `P.padGap` を調整
